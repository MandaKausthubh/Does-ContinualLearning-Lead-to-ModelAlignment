"""
Pipeline for Standard Supervised Fine-Tuning (SFT).

Implements standard fine-tuning on general datasets (Dolly/OASST1)
followed by alignment dataset (StereoSet).
"""
...
def run_sft(
    ...
    Phase 1: Fine-tune on general instruction dataset (Dolly/OASST1)
    Phase 2: Fine-tune on alignment dataset (StereoSet)
    ...
    """
    set_seed(seed)
    logger = setup_logger("sft", output_dir)
    ...
    logger.info("="*60)
    logger.info("Starting Standard Fine-Tuning (SFT)")
    ...
    if not skip_phase2:
        logger.info("\n" + "="*60)
        logger.info("PHASE 2: Alignment Fine-Tuning")
        logger.info("="*60)
    ...
    logger.info(f"\nSFT training complete. Results saved to {output_dir}")
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./experiments/exp_02_sft",
